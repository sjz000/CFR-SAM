...
def parse_args():
    ...
    parser.add_argument("--start-eval", default=200, type=int)
...
def main():
    ...
    for n, p in model.named_parameters():
        if "my_module" not in n and "mask_decoder" not in n and "deep" not in n and "fusion" not in n and "detail" not in n :  #
...
def train_on_epoch(
    ...
    auxiliary_loss = nn.BCEWithLogitsLoss()
    for data_iter_step, (images, true_masks, prompt_points, prompt_labels, all_points, all_points_types, cell_nums, gt_edge) in (
        ...
        loss_dict, loss_dict_refine = criterion(
            outputs,
            true_masks,
        )
        edge_loss = auxiliary_loss(outputs["edge"].squeeze(1), gt_edge.float())
        losses = sum(loss for loss in loss_dict.values()) + sum(loss for loss in loss_dict_refine.values()) + 0.01 * edge_loss
    ...
    with torch.no_grad():
        for data_iter_step, (images, inst_maps, type_maps, prompt_points, prompt_labels, prompt_cell_types,
            ...
            if 'pannuke' in test_dataloader.dataset.dataset:
                ...
                # plt.imshow(inst_maps[0, :, :], cmap='gray')  # 使用灰度图显示掩码
                # plt.title("Binary Mask Visualization")
                # plt.colorbar()  # 添加颜色条
                # plt.show()
                #x = torch.cat(outputs["pred_masks"],dim=0)
                ...
                for batch_ind, file_ind in enumerate(file_inds):
                    ...
                    if cell_nums.sum() > 0:
                        ...
                        mask_data = MaskData(
                            masks=outputs["pred_refine_masks"][batch_inds == batch_ind],
