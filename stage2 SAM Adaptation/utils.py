...
def train_collate_fn(batch):
    images, masks, prompt_points, prompt_labels, all_points, all_points_types, cell_nums, gt_edge = [[] for _ in range(8)]
    for x in batch:
        images.append(x[0])
        masks.append(x[1])
    ...
    return (torch.stack(images), torch.cat(masks), torch.cat(prompt_points), torch.cat(prompt_labels),
            all_points, all_points_types, torch.as_tensor(cell_nums), torch.cat(gt_edge))
    ...
    for (points, labels, cell_types, sub_inds) in batch_iterator(points_per_batch, prompt_points, prompt_labels,
        ...
        if tta:  # used in FullNet and CDNet
            ...
            if dataset == 'monuseg':
                ...
                masks = outputs["pred_refine_masks"]
