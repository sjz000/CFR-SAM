...
class Criterion(nn.Module):
    ...
    def forward(
        ...
        pred_refine = outputs['pred_refine_masks'].unsqueeze(1)
        ...
        loss_dict_refine = {
            'loss_focal': self.focal_loss(pred_refine, true.unsqueeze(1)),
            'loss_dice': self.dice_loss(pred_refine, true),
            'loss_iou': self.iou_loss(pred_refine, true.float(), pred_iou)
        }

        for k in loss_dict:
            loss_dict[k] *= self.loss_weight[k]
        for k in loss_dict_refine:
            loss_dict_refine[k] *= self.loss_weight[k]

        return loss_dict, loss_dict_refine
