...
def simple_visualization(image, points):
    # 复制图像，并将BGR转换为RGB
    img_class = image.copy()
    img_no_class = image.copy()
    ...
    # 绘制点
    for i, class_points in enumerate(points):
        for point in class_points:
            x, y = point
            cv2.circle(img_class, (x, y), 3, colors[i], -1)
            cv2.circle(img_no_class, (x, y), 3, single_color, -1)

    # 转换BGR为RGB后再保存
    img_class_rgb = cv2.cvtColor(img_class, cv2.COLOR_BGR2RGB)
    img_no_class_rgb = cv2.cvtColor(img_no_class, cv2.COLOR_BGR2RGB)

    # 保存
    cv2.imwrite('nuclei_with_class_simple.png', img_class_rgb)
    cv2.imwrite('nuclei_without_class_simple.png', img_no_class_rgb)
    return img_class, img_no_class
...
def predict(
    ...
    # mask = np.load(f'../segmentor/{file.replace("Images", "Masks")[:-4]}.npy', allow_pickle=True)[()]
    # type = mask['type_map']
    # inst = mask['inst_map']
    # points = [[] for _ in range(5)]
    # inst_map = inst
    # type_map = type
    # for pid in np.unique(inst_map)[1:]:  # 遍历每个实例,从中取点
    #     src = ((inst_map == pid) * 255).astype(np.uint8)
    #
    #     contours = cv.findContours(src, cv.RETR_LIST, cv.CHAIN_APPROX_SIMPLE)[0]  # 检测图像中轮廓
